...
namespace FacadeFor3e
    ...
    public class RunProcess : IDisposable
        ...
        private static void ValidateProcess(Process process)
            ...
            if (process.Operations.OfType<DeleteOperation>().Any(op => op.KeySpecification is IdentifyByPosition || op.KeySpecification.GetType().GetGenericTypeDefinition() == typeof(IdentifyByValue<>))
                || process.Operations.OfType<EditOperation>().Any(op => op.KeySpecification is IdentifyByPosition || op.KeySpecification.GetType().GetGenericTypeDefinition() == typeof(IdentifyByValue<>)))
