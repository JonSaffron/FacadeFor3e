5.0.0 (July 2026)
-----------------
- Remove the experimental support for the 3E OData API (this is moved to a separate library)
- Changed the implementation of attribute values from mutable classes to immutable readonly struct
	(this is a breaking change, albeit that it is unlikely to have any impact)
- Update some dependencies to remove security warnings
- Minor platform-specific changes to remove compiler warnings
- For .net standard library 2 only: Support added for getting DateOnly values in GetArchetypeData
	(this is done via reflection so it will be slower then the native support would be)
- Remove support for .net standard 2.1 and .net 6
...
___GetArchetype.GetScalarList is broken in this version___
